...
assert score(test_data, np.array([[40, 60]]), slice(4, 5)) == 500
assert score(test_data, np.array([[40, 60]])) == 57600000
    ...
    for j in range(101 - i):
        for k in range(101 - i - j):
            for m in range(101 - i - j - k):
                # calories
                if i + j + k + m == 100 and score(data, np.array([[i, j, k, m]]), slice(4, 5)) == 500:
