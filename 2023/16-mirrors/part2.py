...
def max_energy(mirrors: list[list[str]]) -> int:
    max_energy = 0
    for x in range(len(mirrors[0])):
        max_energy = max(max_energy, energy(mirrors, (x, 0, 3)))
        max_energy = max(max_energy, energy(mirrors, (x, len(mirrors) - 1, 1)))
    for y in range(len(mirrors)):
        max_energy = max(max_energy, energy(mirrors, (0, y, 0)))
        max_energy = max(max_energy, energy(mirrors, (0, len(mirrors[0]) - 1, 2)))

    return max_energy


test_data = read_data(test_input_file)
assert max_energy(test_data) == 51


data = read_data(input_file)
print(max_energy(data))
